...
{     //FMSC接口初始化
	...
	FSMC_Bank1->BTCR[6]|=1<<0;		//使能BANK1，区域4	  
		/*
		//并口接口初始化
RCC->APB2ENR |= 1<<5;//开启GPIOD的时钟
RCC->APB2ENR |= 1<<7;//开启GPIOF的时钟
RCC->APB2ENR |= 1<<4;//开启GPIOC的时钟
RCC->APB2ENR |= 1<<8;//开启GPIOG的时钟
GPIOD->CRL &= 0x00000000;
GPIOD->CRL |= 0x33333333;//设置GPIOD[7:0]为推挽输出
GPIOD->ODR |= 0x00FF;

GPIOF->CRL &= 0x00000000;
GPIOF->CRL |= 0x33333333;//设置GPIOF[7:0]为推挽输出
GPIOF->ODR |= 0x00FF;

GPIOC->CRH &= 0xFFF000FF;
GPIOC->CRH |= 0x00033300;//设置PC10，PC11，PC12为推挽输出
GPIOC->ODR |= 0x1C00;
GPIOG->CRH &= 0x00FFF00F;
GPIOG->CRH |= 0x00000330;//设置PG10，PG9推挽输出
GPIOG->ODR |= 3<<9;
*/
	    //	LCD_REST= 0 ;
		//	delay_us(100000);
	...
	}else if(lcddev.id==0x9328)//ILI9328   OK  
	{
        ...
        LCD_WriteReg(0x0008,0x0202);	           
        LCD_WriteReg(0x0009,0x0000);         
        LCD_WriteReg(0x000a,0x0000);//display setting  8       
     ...
     // LCD_WriteReg(0x0012,0x001d); 			 
        ...
        LCD_WriteReg(0x0013,0x1a00);   
				// LCD_WriteReg(0x0013,0x1500);  //8
        LCD_WriteReg(0x0029,0x0006);
				// LCD_WriteReg(0x0029,0x0018);//8
        LCD_WriteReg(0x002b,0x000d);
   	delay_us(50000); 
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
 	...
 	delay_us(200000); // Dis-charge capacitor power voltage
