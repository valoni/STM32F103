...
const SPI_CONFIGURATION g_W25Q64_spi = {
    (GPIO_PIN)(16 + 12),   // DeviceCS       // CS on pin PB12
    ...
    1,                    // SPI_mod        // device on SPI1
    {GPIO_PIN_NONE,FALSE} // BusyPin        // none
};


BOOL W25Q64_EnableWrite()
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    
    BYTE en_cmd = WrEn_Instr;
    return CPU_SPI_nWrite8_nRead8(g_W25Q64_spi, &en_cmd, 1, NULL, 0, 0);
}
    
BOOL W25Q64_WaitReady()
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    do {
        if (!CPU_SPI_nWrite8_nRead8(g_W25Q64_spi, &rd_cmd, 1, &status, 1, 1)) return FALSE;
...
//--//
    ...
    //
BOOL W25Q64_Flash_Driver::ChipInitialize( void* context )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //
BOOL W25Q64_Flash_Driver::ChipUnInitialize( void* context )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //
const BlockDeviceInfo* W25Q64_Flash_Driver::GetDeviceInfo( void* context )
{
    ...
    //   
BOOL  W25Q64_Flash_Driver::Read( void* context, ByteAddress StartSector, UINT32 NumBytes, BYTE * pSectorBuff)
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    BYTE rd_cmd[4] = {Read_Instr,
                      ...
                      (BYTE)(StartSector >> 8),
                      (BYTE)StartSector};
    return CPU_SPI_nWrite8_nRead8(g_W25Q64_spi, rd_cmd, 4, pSectorBuff, NumBytes, 5);
}
    ...
    //   This function reads the number of sectors specified from the device.
    //   
BOOL W25Q64_Flash_Driver::Write(void* context, ByteAddress Address, UINT32 NumBytes, BYTE * pSectorBuff, BOOL ReadModifyWrite)
{
    ...
    while(NumBytes > 0) {
        ...
        // ready?
        if (!W25Q64_EnableWrite()) return FALSE;
        // write sector
        if (!CPU_SPI_nWrite8_nRead8(g_W25Q64_spi, wr_cmd, len + 4, NULL, 0, 0)) return FALSE;
        if (!W25Q64_WaitReady()) return FALSE;
        Address += len;
        NumBytes -= len;
    }
...
BOOL W25Q64_Flash_Driver::Memset(void* context, ByteAddress Address, UINT8 Data, UINT32 NumBytes)
{
    NATIVE_PROFILE_PAL_FLASH();
...
BOOL W25Q64_Flash_Driver::GetSectorMetadata(void* context, ByteAddress SectorStart, SectorMetadata* pSectorMetadata)
{
    // no metadata
    return FALSE;
}

BOOL W25Q64_Flash_Driver::SetSectorMetadata(void* context, ByteAddress SectorStart, SectorMetadata* pSectorMetadata)
{
    // no metadata
    return FALSE;
}
    ...
    //    
BOOL W25Q64_Flash_Driver::IsBlockErased( void* context, ByteAddress BlockStart, UINT32 BlockLength )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //    
BOOL W25Q64_Flash_Driver::EraseBlock( void* context, ByteAddress Sector )
{
    NATIVE_PROFILE_HAL_DRIVERS_FLASH();
    ...
    //return CPU_SPI_nWrite8_nRead8(g_W25Q64_spi, er_cmd, 4, NULL, 0, 0);
    if (!CPU_SPI_nWrite8_nRead8(g_W25Q64_spi, er_cmd, 4, NULL, 0, 0)) return FALSE;
    return W25Q64_WaitReady();
}
    ...
    //
void  W25Q64_Flash_Driver::SetPowerState( void* context, UINT32 State )
{
...
UINT32 W25Q64_Flash_Driver::MaxSectorWrite_uSec( void* context )
{
    NATIVE_PROFILE_PAL_FLASH();
...
UINT32 W25Q64_Flash_Driver::MaxBlockErase_uSec( void* context )
{
    NATIVE_PROFILE_PAL_FLASH();
...
#pragma arm section rodata = "g_W25Q64_Flash_DeviceTable"
#endif

struct IBlockStorageDevice g_W25Q64_Flash_DeviceTable = 
{                          
    &W25Q64_Flash_Driver::ChipInitialize,
    &W25Q64_Flash_Driver::ChipUnInitialize,
    &W25Q64_Flash_Driver::GetDeviceInfo,
    &W25Q64_Flash_Driver::Read,
    &W25Q64_Flash_Driver::Write,
    &W25Q64_Flash_Driver::Memset,    
    &W25Q64_Flash_Driver::GetSectorMetadata,
    &W25Q64_Flash_Driver::SetSectorMetadata,
    &W25Q64_Flash_Driver::IsBlockErased,
    &W25Q64_Flash_Driver::EraseBlock,
    &W25Q64_Flash_Driver::SetPowerState,
    &W25Q64_Flash_Driver::MaxSectorWrite_uSec,
    &W25Q64_Flash_Driver::MaxBlockErase_uSec,    
};
