...
#include <tinyhal.h>
#include "..\..\DeviceCode\Targets\Native\LPC24XX\DeviceCode\LPC24XX.h"
...
void ApplicationEntryPoint()
{
	...
	wait_joystick = true;
	UINT32 pins[] = {12,15,24,25,26,89};
	for(UINT32 pin = 0; pin < 6; pin++)
	{
		if(!g_LPC24XX_GPIO_Driver.EnableInputPin( pins[pin], false,  joystickISR, NULL, GPIO_INT_EDGE_HIGH, (GPIO_RESISTOR)2 ))
		{
   ...
   while(1)
     {
		...
		ByteAddress PhyAddress = (ByteAddress) 0x8100000C;
		
		SectorAddress SectAddress = 0x8100000C;
		UINT32 RangeIndex;
		UINT32 RegionIndex;
		
		const BlockRegionInfo *pBlockRegionInfo;
		SST->FindForBlockUsage(	/*UINT32*/ BlockUsage::ANY , 
								  PhyAddress , RegionIndex, RangeIndex );

		if(PhyAddress == 0x8100000C)
		{
		...
		if(!SST->IsBlockErased(0x8100000C, 32*1024))
		{
		...
		//ERASE before writing!
		if(!SST->IsBlockErased(0x8100800C, 32*1024))
		{
			debug_printf( "Erasing block " );
			if(!SST->EraseBlock(0x81008000))
			{
				debug_printf( "failed.\r\n" );
				break;
			}
		...
		if(SST->Write(/*UINT32*/ 0x81008000, /*UINT32 NumOfBytes*/ 16, /*BYTE* */ pSectorBuf, /*SectorMetadata* */ FALSE))
		{
			debug_printf( "Correctly written 16 bytes to Sector 0x%08x\r\n", SectAddress);
		}
    ...
    CPU_GPIO_DisablePin(121, RESISTOR_DISABLED, GPIO_ATTRIBUTE_NONE, GPIO_ALT_MODE_1 );

    LPC24XX::PCB().Regs[7].PINSEL=0x00000000;       												/* 设定P3口的连接方式  		*/
    while(1)
    {
		...
		CPU_GPIO_SetPinState(121,1);
		sysDelayNS(50);
		CPU_GPIO_SetPinState(121,0);
		sysDelayNS(50);
    }
