...
PSR_MODE_USER       EQU     0x10
PSR_MODE_FIQ        EQU     0x11
PSR_MODE_IRQ        EQU     0xd2
PSR_MODE_SUPERVISOR EQU     0xd3
PSR_MODE_ABORT      EQU     0x17
PSR_MODE_UNDEF      EQU     0x1B
PSR_MODE_SYSTEM     EQU     0x1F
...
EntryPoint
    ...
    msr     cpsr_cxsf, #PSR_MODE_SYSTEM    ; go into System mode, interrupts off
...
PreStackInit_Exit_Pointer 
    ...
    ldr     r1, =StackTop               ; new SYS stack pointer for a full decrementing stack


   msr     cpsr_cxsf, #PSR_MODE_ABORT     ; go into ABORT mode, interrupts off
    mov     sp, r1                      ; stack top
    sub     r0, r1, #STACK_MODE_ABORT   ; ( take the size of abort stack off )
    
    msr     cpsr_cxsf, #PSR_MODE_UNDEF     ; go into UNDEF mode, interrupts off
    mov     sp, r1                      ; stack top - abort stack 
    sub     r1, r1, #STACK_MODE_UNDEF   ; 
    
    
    IF :DEF: FIQ_SAMPLING_PROFILER
    msr     cpsr_cxsf, #PSR_MODE_FIQ       ; go into FIQ mode, interrupts off
    mov     sp, r1                      ; stack top - abort stack - undef stack
    sub     r1, r1, #STACK_MODE_FIQ 
    ENDIF    

    msr     cpsr_cxsf, #PSR_MODE_IRQ       ; go into IRQ mode, interrupts off
    mov     sp, r1                      ; stack top - abort stack - undef stack (- FIQ stack)
    sub     r1, r1, #STACK_MODE_IRQ
   
    msr     cpsr_cxsf, #PSR_MODE_SYSTEM    ; go into System mode, interrupts off
    mov     sp,r1                       ; stack top - abort stack - undef stack (- FIQ stack) - IRQ stack
