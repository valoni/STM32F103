...
BOOL S3C2416_GPIO_Driver::Initialize()
{
   ...
   // S3C2416  只有GPF和GPG才有中断
	...
	EINTMASK_REG = 0x00fffff0; //关闭所有中断
    ...
    if(!CPU_INTC_ActivateInterrupt( S3C2416_AIC::INT_INDEX_EINT0, ISR, NULL)) return FALSE;
    if(!CPU_INTC_ActivateInterrupt( S3C2416_AIC::INT_INDEX_EINT1, ISR, NULL)) return FALSE;
    if(!CPU_INTC_ActivateInterrupt( S3C2416_AIC::INT_INDEX_EINT2, ISR, NULL)) return FALSE;
    if(!CPU_INTC_ActivateInterrupt( S3C2416_AIC::INT_INDEX_EINT3, ISR, NULL)) return FALSE;
    if(!CPU_INTC_ActivateInterrupt( S3C2416_AIC::INT_INDEX_EINT4_7, ISR, NULL)) return FALSE;
    if(!CPU_INTC_ActivateInterrupt( S3C2416_AIC::INT_INDEX_EINT8_23, ISR, NULL)) return FALSE;
...
BOOL S3C2416_GPIO_Driver::Uninitialize()
{
    ...
    // uninitialize the interrupt information
    {
		...
		*/
			EINTMASK_REG = 0x00fffff0; //关闭所有中断
			EINTPEND_REG = 0x0; //清除所有中断标志位
    }

    if(!CPU_INTC_DeactivateInterrupt( S3C2416_AIC::INT_INDEX_EINT0 )) return FALSE;
    if(!CPU_INTC_DeactivateInterrupt( S3C2416_AIC::INT_INDEX_EINT1 )) return FALSE;
    if(!CPU_INTC_DeactivateInterrupt( S3C2416_AIC::INT_INDEX_EINT2 )) return FALSE;
    if(!CPU_INTC_DeactivateInterrupt( S3C2416_AIC::INT_INDEX_EINT3 )) return FALSE;
    if(!CPU_INTC_DeactivateInterrupt( S3C2416_AIC::INT_INDEX_EINT4_7 )) return FALSE;
    if(!CPU_INTC_DeactivateInterrupt( S3C2416_AIC::INT_INDEX_EINT8_23 )) return FALSE;
...
void S3C2416_GPIO_Driver::DisablePin( GPIO_PIN pin, GPIO_RESISTOR resistorState, UINT32 dir_out, GPIO_ALT_MODE function )
{
   ...
   // UINT32  bitmask_pullup = 0x02 << (PinToBit ( pin ) * 2); //GPIOUDP用两个Bit来控制下拉
    ...
    S3C2416_GPIO_TYPE &GPIOX  = *(S3C2416_GPIO_TYPE*)GetPort(port);
	...
	*/
  SetResistor(pin,resistorState); //��������������
	
	UINT32  bitmask = 1 << PinToBit ( pin ) ; //GPIOSEL用1Bit来Selects the function of port
    switch (function)
    {
    ...
    if(GetPort(port) == GPIOF)
    	{ 
    	EINTMASK_REG &= ~bitmask;
			}
	else if(GetPort(port) == GPIOG)
		{
        EINTMASK_REG &= ~(bitmask << 8);
	   }
...
void S3C2416_GPIO_Driver::EnableOutputPin( GPIO_PIN pin, BOOL initialState )
{
    ...
    UINT32  bitmask_output  = 0x01 << (PinToBit( pin )*2);
    UINT32  bitmask  = 0xFF << (PinToBit ( pin ) * 2); //GPIOUDP用两个Bit来控制关闭
    UINT32  bitmask_output_enable  = 0x1 << PinToBit( pin );
	UINT32  port = PinToPort( pin );
    S3C2416_GPIO_TYPE &GPIOX  = *(S3C2416_GPIO_TYPE*)GetPort(port);
   ...
   // pioX.PIO_PER = bitmask;                   // Enable PIO function
    GPIOX.GPIOCON &= ~bitmask;
    GPIOX.GPIOCON |= bitmask_output;       //GPIO set Output
    
        if(initialState)
       // pioX.PIO_SODR = bitmask;
       GPIOX.GPIODAT |= bitmask_output_enable;
    else
...
/*
    UINT32  bitmask_input  = 0x00 << (PinToBit( pin )*2);
    UINT32  bitmask  = 0xFF << (PinToBit ( pin ) * 2); //GPIOUDP用两个Bit来控制关闭
    UINT32  bitmask_pullup = 0x02 << (PinToBit ( pin ) * 2); //GPIOUDP用两个Bit来控制下拉
	UINT32  bitmask_pulldown = 0x01 << (PinToBit ( pin ) * 2); //GPIOUDP用两个Bit来控制上拉
	UINT32  bitmask_pulldisable = 0 << (PinToBit ( pin ) * 2); //GPIOUDP用两个Bit来控制关闭
    ...
    switch (resistorState)
    {
        case RESISTOR_DISABLED:
            GPIOX.GPIOUDP &= bitmask;     // Disable the pull up resistor
            break;
        case RESISTOR_PULLUP:
		   GPIOX.GPIOUDP &= bitmask;
           GPIOX.GPIOUDP |= bitmask_pullup;        // Enable the pull up resistor
            break;
        case RESISTOR_PULLDOWN:
           GPIOX.GPIOUDP &= bitmask;
           GPIOX.GPIOUDP |= bitmask_pulldown;   // Enable the  pulldown resistors 
           break;
	}
    PIN_ISR_DESCRIPTOR &pinIsr = g_S3C2416_GPIO_Driver.m_PinIsr[ pin ];
