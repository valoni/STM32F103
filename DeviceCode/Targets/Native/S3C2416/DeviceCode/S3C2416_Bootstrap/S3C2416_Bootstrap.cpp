...
void S3C2416::Debug_SendByte(unsigned char data)
{
	(*(volatile unsigned char *)0x50000028)=0x28;
	while(!((*(volatile unsigned *)0x50000010) & 0x2));   //Wait until THR is empty.
	(*(volatile unsigned char *)0x50000020)=(unsigned char)(data);
}


void S3C2416::Debug_printf( const char* format, ... )
{
	char    buffer[256];
	va_list arg_ptr;
	...
	int i;
	for (i = 0; i < len; i++)
		S3C2416::Debug_SendByte((unsigned char)buffer[i]);
}
...
{
	S3C2416::Debug_printf("Bootstrap - CPU_IsMMUEnabled: %1d\r\n", CPU_IsMMUEnabled());
	...
	ARM9_MMU::GenerateL1_Sections(
	  ...
	  //	c_Bootstrap_SRAM_End,                                                 //SDRAM MVA 0x0
