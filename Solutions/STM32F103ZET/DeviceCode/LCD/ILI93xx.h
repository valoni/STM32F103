...
/*
//并口，8位数据
//#define Bus_16        //16?????,????8???,??????,????16???,?????,??8?????,????????????8?????
#define  LCD_DataPortH(x) GPIOD->ODR=(GPIOD->ODR&0xFF00)|(x&0x00FF) //PB0~PB7
//#define  LCD_DataPortL(x) GPIOF->ODR=(GPIOF->ODR&0xFF00)|(x&0x00FF)   
   
#define  LCD_RS   PCout(11)  		 //数据/命令切换
#define  LCD_WR   PCout(12)		  //写控制
#define  LCD_RD   PCout(10)		     //读控制
#define  LCD_CS   PGout(10)		//片选
#define  LCD_REST PGout(9)	      //复位 
//IO方向设置
#define LCD_IO_IN()  {GPIOD->CRL&=0X00000000;GPIOD->CRL|=0x88888888;}
#define LCD_IO_OUT() {GPIOD->CRL&=0X00000000;GPIOD->CRL|=0x33333333;}
//////////////////////////////////////////////////////////////////////////////////
*/	 
//扫描方向定义
