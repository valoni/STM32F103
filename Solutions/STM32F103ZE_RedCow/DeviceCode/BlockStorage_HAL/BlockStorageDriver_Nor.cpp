...
{	
	...
	{
		uint16_t buf;
		FSMC_NOR_ReadBuffer(&buf, norAddr - 1, 1);
	...
	if(dwReadWord != 0)
	{
		FSMC_NOR_ReadBuffer(reinterpret_cast<uint16_t *>(pSectorBuff), norAddr + dwIndex, dwReadWord);
	}
	...
	if(NumBytes % 2 != 0)
	{
		//Read the last word
		uint16_t buf = 0;
		FSMC_NOR_ReadBuffer(&buf, norAddr + dwReadWord + dwIndex, 1);
...
BOOL BlockStorageDriver_Nor::Write(void* context, ByteAddress Address, UINT32 NumBytes, BYTE * pSectorBuff, BOOL ReadModifyWrite)
{
	...
	if(norAddr % 2 != 0)
	{
		...
		FSMC_NOR_WriteBuffer(&buf, norAddr, 1);
	}
	
	DWORD dwWriteWord = (NumBytes - dwIndex)/ 2;
	if(dwWriteWord != 0)
	{
		FSMC_NOR_WriteBuffer(reinterpret_cast<uint16_t *>(pSectorBuff), norAddr + dwIndex, dwWriteWord);
	}
	...
	if(NumBytes % 2 != 0)
	{
		//Read the last word
		uint16_t buf = 0;
		FSMC_NOR_ReadBuffer(&buf, norAddr + dwWriteWord + dwIndex, 1);
		...
		FSMC_NOR_WriteBuffer(&buf, norAddr + dwWriteWord + dwIndex, 1);
	}
