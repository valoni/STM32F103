...
{ 	
	...
	GPIO_Set(GPIOF,PIN0,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);							//PF0,FSMC_A0
	GPIO_Set(GPIOG,PIN10,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_100M,GPIO_PUPD_PU);							//PG10,FSMC_NE3
 	...
 	GPIO_AF_Set(GPIOG,10,12);	//PG10,AF12
	...
	{
		//重新配置写时序控制寄存器的时序   	 							    
		FSMC_Bank1E->BWTR[4]&=~(0XF<<0);//地址建立时间(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[4]&=~(0XF<<8);//数据保存时间清零
		FSMC_Bank1E->BWTR[4]|=3<<0;		//地址建立时间(ADDSET)为3个HCLK =18ns  	 
		FSMC_Bank1E->BWTR[4]|=2<<8; 	//数据保存时间(DATAST)为6ns*3个HCLK=18ns
	}else if(lcddev.id==0X6804||lcddev.id==0XC505)	//6804/C505速度上不去,得降低
	{
		//重新配置写时序控制寄存器的时序   	 							    
		FSMC_Bank1E->BWTR[4]&=~(0XF<<0);//地址建立时间(ADDSET)清零 	 
		FSMC_Bank1E->BWTR[4]&=~(0XF<<8);//数据保存时间清零
		FSMC_Bank1E->BWTR[4]|=10<<0;	//地址建立时间(ADDSET)为10个HCLK =60ns  	 
		FSMC_Bank1E->BWTR[4]|=12<<8; 	//数据保存时间(DATAST)为6ns*13个HCLK=78ns
	}
//	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID  
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		delay_us(2000); // Dis-charge capacitor power voltage
		LCD_WriteReg(0x0010, 0x1690); // SAP, BT[3:0], AP, DSTB, SLP, STB
		LCD_WriteReg(0x0011, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
		delay_us(500); // Delay 50ms
