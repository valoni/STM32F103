...
void K9F2G_8_BS_Driver::WaitReady()
{
    ...
    while (!(nandflash.NFSTAT &NFSTAT_RnB));
}
...
BOOL K9F2G_8_BS_Driver::ChipInitialize( void* context )
{
    ...
    if (0x00 == ManufacturerCode)
    {
        S3C2416::Debug_printf( "Flash_ChipInitialize: ManufacturerCode failure!\r\n" );
        return FALSE;
    }
   else{
	    S3C2416::Debug_printf( "Flash_ChipInitialize: ManufacturerCode OK!ManufacturerCode is %d\r\n",ManufacturerCode );
   }
   
    if (0x00 == DeviceCode)
    {
        S3C2416::Debug_printf( "Flash_ChipInitialize: DeviceCode failure!\r\n" );
        return FALSE;		
    }
	else{
		S3C2416::Debug_printf( "Flash_ChipInitialize: DeviceCode OK! DeviceCode is %d\r\n" ,DeviceCode);
	}
    return TRUE;
